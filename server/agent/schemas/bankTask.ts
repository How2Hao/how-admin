...
export const BankTask = z.object({
  ...
  ruleBrief: z.string().describe('活动简要描述。简述活动机制即可，避免照抄大段细则。'),
  ...
  benefitCategoryId: z.string().describe('优惠分类ID。必须使用 `get_benefit_category_id` 工具根据文章中明确的优惠分类名称，并从候选结果中选择最匹配的一项。'),
  benefitPlatformId: z.string().describe('优惠平台ID。必须使用 `get_benefit_platform_id` 工具根据文章中明确的优惠平台名称，并从候选结果中选择最匹配的一项。'),
})
