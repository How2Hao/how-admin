...
export default defineHandler(async (event) => {
  ...
  const [existing] = await db.select().from(appRelease).where(eq(appRelease.id, id)).limit(1)
  ...
  try {
    await db.update(appRelease).set(update).where(eq(appRelease.id, id))
    return { ok: true }
  }
  catch (e: any) {
    if (e?.code === 'ER_DUP_ENTRY') throw createError({ statusCode: 409, statusMessage: 'version 已存在' })
