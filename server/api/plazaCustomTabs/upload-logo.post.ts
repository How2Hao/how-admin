...
import { uploadPlazaTabLogo } from '~~/utils/ossClient'
...
export default defineHandler(async (event) => {
  ...
  if (buf.byteLength > MAX_BYTES) throw createError({ statusCode: 413, statusMessage: `图片过大 (${(buf.byteLength / 1024 / 1024).toFixed(2)} MB)，logo 请控制在 2MB 内` })

  try {
    const url = await uploadPlazaTabLogo(buf, ext)
